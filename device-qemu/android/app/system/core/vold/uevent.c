...
#ifdef ADD_ISO
    { "dummy_switch", handle_switch_event }, 
#else
...
int process_uevent_message(int socket)
{
    ...
    while (s < end) {
        if (first) {
            ...
            first = 0;
#if 1
    LOGI("process_uevent_message:path=%s\n", event->path);
#endif
        } else {
#if 1
    LOGI("process_uevent_message:s=%s\n", s);
#endif
