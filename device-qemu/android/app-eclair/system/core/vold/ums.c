...
#ifdef ADD_ISO
int ums_enable(char *dev_fspath, char *lun_syspath, int isSd)
#else
...
#endif
{
    ...
    memset(writeData, 0, 4096);
    if (isSd) {
    	sprintf(writeData, "SD:%s", dev_fspath);
    } else {
    	sprintf(writeData, "CD:%s", dev_fspath);
    }
    LOG_VOL("ums_enable():writeData=%s\n", writeData);
#endif
